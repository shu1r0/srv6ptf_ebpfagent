...
sudo ip netns exec h2 ip -6 addr add 2001:db8:20::100/48 dev h2_r1
sudo ip netns exec r1 ip -6 route add 2001:db8:20::100/128 encap seg6 mode encap segs 2001:db8:30::3 via 2001:db8:30::3 dev r1_h1
sudo ip netns exec r1 tcpdump -i r1_h1 -w r1_h1.pcap &
# sudo ip netns exec r1 echo 200 test >> /etc/iproute2/rt_tables
# sudo ip netns exec r1 ip -6 route add 2001:db8:30::3/128 encap seg6local action end dev r1_h1 table 200
# sudo ip netns exec r1 ip -6 rule add to 2001:db8:30::3/128 table 200
sudo ip netns exec r1 ip6tables -A POSTROUTING -t mangle -d 2001:db8:30::3/128 --goto PREROUTING
