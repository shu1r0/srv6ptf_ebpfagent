...
static __always_inline bool push_pktidtlv_skb(struct __sk_buff *skb, int tlv_off, int nodeid, int counter){
  ...
  // adjust ipv6 (おそらく，拡張ヘッダなしのやつかなって感じ)
...
// Ingress Prog
SEC("xdp") int ingress(struct xdp_md *ctx) 
{
  ...
  if(tlv){
    ...
    // __u32 node_id = 1;
    if(node_id == NULL){
      bpf_info("Ingress: Node id is not found in Map.");
      return XDP_PASS;
    }
    __u32 *count = bpf_map_lookup_elem(&counter_map, &counter_index);
    // __u32 count = 1;
    if(count == NULL){
      bpf_info("Ingress: Counter is not found in Map.");
      return XDP_PASS;
    }
...
// Egress Prog
SEC("tc") int egress(struct __sk_buff *skb) 
{
  ...
  if(tlv){
    ...
    __u32 *node_id = bpf_map_lookup_elem(&config_map, &node_id_index);
    if(node_id == NULL){
      bpf_debug("Egress: Node id is not found in Map.");
      return TC_ACT_OK;
    }

    __u32 *counter = bpf_map_lookup_elem(&config_map, &node_id_index);
    if(counter == NULL){
      bpf_debug("Egress: Counter is not found in Map.");
      return TC_ACT_OK;
    }
